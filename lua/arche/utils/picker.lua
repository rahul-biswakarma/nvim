...
function telescopePickers.prettyFilesPicker(pickerAndOptions)
    ...
    options.entry_maker = function(line)
        ...
        originalEntryTable.display = function(entry)
            ...
            local tail, pathToDisplay = telescopePickers.getPathAndTail(
                                            entry.value)
...
function telescopePickers.prettyGrepPicker(pickerAndOptions)
    ...
    local originalEntryMaker =
        telescopeMakeEntryModule.gen_from_vimgrep(options)
    ...
    options.entry_maker = function(line)
        ...
        originalEntryTable.display = function(entry)
            ...
            if not options.disable_coordinates then
                if entry.lnum then
                    if entry.col then
                        coordinates = string.format(" -> %s:%s", entry.lnum,
                                                    entry.col)
                    else
    ...
    if pickerAndOptions.picker == 'live_grep' then
        -- Modify the options to only search in the content of the files
        options.search = {content = true}
...
function telescopePickers.prettyDocumentSymbols(localOptions)
    ...
    local originalEntryMaker = telescopeMakeEntryModule.gen_from_lsp_symbols(
                                   options)
    ...
    options.entry_maker = function(line)
        ...
        local displayer = telescopeEntryDisplayModule.create({
            separator = ' ',
            items = {
                {width = fileTypeIconWidth}, {width = 20}, {remaining = true}
            }
        })
...
function telescopePickers.prettyWorkspaceSymbols(localOptions)
    ...
    options.entry_maker = function(line)
        ...
        local displayer = telescopeEntryDisplayModule.create({
            separator = ' ',
            items = {
                {width = fileTypeIconWidth}, {width = 15}, {width = 30},
                {width = nil}, {remaining = true}
            }
        })
        ...
        originalEntryTable.display = function(entry)
            ...
            return displayer {
                string.format("%s", kind_icons[(entry.symbol_type:lower()
                                  :gsub("^%l", string.upper))]),
                {entry.symbol_type:lower(), 'TelescopeResultsVariable'},
                {entry.symbol_name, 'TelescopeResultsConstant'}, tailForDisplay,
                {pathToDisplay, 'TelescopeResultsComment'}
            }
        end
...
function telescopePickers.prettyBuffersPicker(localOptions)
    ...
    options.entry_maker = function(line)
        ...
        local displayer = telescopeEntryDisplayModule.create {
            separator = " ",
            items = {
                {width = fileTypeIconWidth}, {width = nil}, {width = nil},
                {remaining = true}
            }
        }
